/**
 * grade-calc.js — 加權成績試算純函式庫
 *
 * 職責邊界:比照 credit-calc.js,只做「輸入資料 → 算出數字」的純函式,
 * 不碰 localStorage、不碰GitHub、不碰DOM。被 modules/grades.html 使用。
 *
 * 計分規則(明訂於此,供UI層與測試對照):
 *   - 只計入 status="selected"(已選,非模擬預選)且 resultStatus !== "withdrawn"(非退選)
 *     且已填分數(score為有效數字)的課程。
 ...
 *     兩者都以「學分」加權,與credit-calc.js的計分口徑一致。
 *
 * 用法:
 *   GradeCalc.computeSemesterSummary(myCourses, '大二上')
 *     → { avgScore, gpa, totalCredits, courseCount } (無合格資料時avgScore/gpa為null)
 */
(function (global) {
  "use strict";
